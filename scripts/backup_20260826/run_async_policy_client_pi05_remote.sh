...
#   SKIP_MOVE_TO_START=true bash scripts/run_async_policy_client_pi05_remote.sh configs/softfold_piper_pi05.json
set -euo pipefail
...
if [[ "${REMOTE_USE_SSH_TUNNEL}" == "true" ]]; then
  echo "Prerequisites:"
  echo "  1) On ${REMOTE_SSH_HOST}: bash scripts/start_policy_server_pi05.sh ${CONFIG_PATH}"
